// tonal js does most of this...

$(function(){

	keys = [];
	var jj = music.notes.actions.generateKey({'key':'F#'});
	var k = music.notes.actions.checkSpellings(jj);

	$.each(music.keys.sharps, function(k,v) {
	 var jj = music.notes.actions.generateKey({'key': v});
	 var k = music.notes.actions.checkSpellings(jj,'sharp');
	 keys.push(k);
	});
	
	$.each(music.keys.flats, function(k,v) {
	 var jj = music.notes.actions.generateKey({'key': v});
	 var k = music.notes.actions.checkSpellings(jj,'flat');
	...
	$('#btn1').click(function(){
		var q = music.chords.newTriad(keys[Math.floor(Math.random() *15)], Math.floor(Math.random() * 7));
		console.log(q);
		$('#chord').html(q['random triad'].join(','));
		$('#answer').html(q['root'] + " " + q['triadFcn']);
	});


	// // generate harmonic minor keys.
	// $.each(music.keys.sharps, function(k,v) {
	//  console.log('harmonic minor sharps');
	//  var jj = music.notes.actions.generateKey({'key': v});
	//  var k = music.notes.actions.checkSpellings(jj,'sharp');
	//  var hm = music.notes.actions.hmFromMajor(k);
	//  var mm = music.notes.actions.mmFromMajor(k);
	//  console.log(jj);
	//  console.log(hm);
	//  console.log(mm);
	//  // keys.push(k);
